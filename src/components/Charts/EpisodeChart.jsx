...
export default function EpisodeChart(props) {
  ...
  const previous = () => {
    if (current > 1) {
      Plotly.restyle(episodePlotRef.current, {
        x: [data[current - 2][0]["x"]],
        y: [data[current - 2][0]["y"]],
      });
  ...
  return (
    <div className={"h-full w-full tracking-tight bg-white z-[999]"}>
        ...
        {Object.keys(episodeData).map((key, idx) => {
          return (
            <div className="flex-1 items-center justify-center px-1" key={idx}>
              <div className="text-sm text-drawerText capitalize-first-letter">
                {key}
              </div>
              <div className="text-sm text-drawerTextDark">
                {episodeData[key]}
              </div>
            </div>
          );
      ...
      <div className="flex flex-row bg-borderPrimary m-2 px-2 py-1">
        <div className="flex text-drawerTextDark text-sm">
        ...
        <div className="flex text-drawerText mx-1">
          {current}/{total}
        </div>
        <button
          type="button"
      ...
      <div id="episodeChart" ref={episodePlotRef} className="flex h-[100px]" />
