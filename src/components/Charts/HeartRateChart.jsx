...
const HeartRateChart = () => {
  ...
  const [episodeChartData, setEpisodeChartData] = useState(useSelector((state) => state.episodeData.episodeChartData));
  const [selectedChartData, setSelectedChartData] = useState(useSelector((state) => state.episodeData.selectedChartData));

  const [showHrNotification, setHrNotification] = useState(episodeDataStore.showHrNotification);
  ...
  const updateSelectedChart = (val) => {
    if (val) {
      ...
      setSelectedChartData(episodeChartData[val]);
      setSelectedChartData({...selectedChartData, show: true});
      Object.keys(episodeChartData).map((item) => Object.assign(item, { test: false }));
    } else {
      setSelectedChartData({...selectedChartData, show: false});
    }
  ...
  return (
    <div className={'w-full pt-1'}>
      <div className='flex-1 w-full h-full'>
        <div className="mt-1 border-b-4 border-[#4A5060]"/>
        <div className='flex justify-between bg-white px-3 py-1 outline outline-1 outline-[#D9D9D9]'>
          ...
          <div className={'flex items-center cursor-pointer w-[40px]'}>
            <BsBell className={'text-xl text-[rgba(0,0,0,0.6)] hover:text-[rgba(0,0,0,1)]'} onClick={()=>setHrNotification(true)} />
            <HiOutlineAdjustmentsVertical className={'text-xl text-[rgba(0,0,0,0.6)] hover:text-[rgba(0,0,0,1)]'} onClick={()=>setHrNotification(true)} />
          </div>
        </div>
        <div className="border-b-4 border-[#4A5060]"/>
        <div className='flex justify-between bg-white px-3 py-1 outline outline-1 outline-[#D9D9D9]'>
          <div className='w-full grid grid-cols-6'>
        ...
        <div id="heartRateChart" ref={heartPlotRef} className={'w-full h-[20vh] outline outline-1 outline-[#D9D9D9]'} />
      </div>
    </div>
  );
};
